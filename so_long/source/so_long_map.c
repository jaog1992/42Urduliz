...
int	numero_de_lineas(Mapa *mapa1, char *argv)
{
	char	*line = NULL;
	int		fd;
...
int	parse_file(Mapa *mapa1, char **ptr, int fd, char **argv, int i)
{
...
int	main(int argc, char **argv)
{
	...
	if (argument_checker(argc, argv) == 1)
        return (0);
	free_flag = parse_file(mapa1, ptr, 0, argv, 0);
	if	(free_flag > 0)
		return (free_ptr(mapa1,ptr));
	return (0);
}